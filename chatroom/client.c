...
void run_client(const char *host, const char *port, const char *username) {
  ...
  // connect
  int t = connect(serverSocket, result->ai_addr, result->ai_addrlen);
  if (t != 0) {
    perror("connect");
    exit(1);
  }
  ...
  // read from server
  pthread_create(threads, NULL, read_from_server, (void*)NULL);
  ...
  void *result_1 = NULL;
  void *result_2 = NULL;
  pthread_join(threads[0], &result_1);
  pthread_join(threads[1], &result_2);
